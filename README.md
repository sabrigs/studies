# About

This repository contains books, courses, videos, and other resources that are part of my learning journey in software development.

The goal is to keep everything organized in a single place and track my progress over time.

I decided to keep these materials public because every programming journey is different. In the same way I learned from and felt inspired by other developers sharing their paths, I hope this repository can also help someone through the resources, notes, and references collected here.

---

# Books

- [Grokking Algorithms](https://www.amazon.com.br/Grokking-Algorithms-illustrated-programmers-curious-ebook/dp/B09781V6F7/?_encoding=UTF8&pd_rd_w=aBQ7h&content-id=amzn1.sym.114e2c84-01f5-48f9-9ca4-14fe60e0e43a&pf_rd_p=114e2c84-01f5-48f9-9ca4-14fe60e0e43a&pf_rd_r=143-5563191-6818913&pd_rd_wg=9C33v&pd_rd_r=22af3143-d2f5-444b-911f-f8f60f2011c0)

---

# Courses
- [CS50x](https://cs50.harvard.edu/x/)
- [Learn Memory Management in C (Boot.dev)](https://www.boot.dev/courses/learn-memory-management-c)
- [Software Engineering College](https://estacio.br/cursos/graduacao/engenharia-de-software?srsltid=AfmBOoobu7oWiV_SkgLw9-xJxt_MKDETQxDFnynoqoNaNFRoZWq0mhT2)
## Git
- [Git By Bit](https://gitbybit.com/)
- [Learn Git (Boot.dev with ThePrimeagen)](https://www.boot.dev/courses/learn-git)